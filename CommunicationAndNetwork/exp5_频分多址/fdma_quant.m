...
ber=zeros(N_u,size(n0,2));
ber_theory=zeros(N_u,size(n0,2));
...
ber_dB=10*log10(ber);
ber_theory_dB=10*log10(ber_theory);
SNRdB=10*log10(Eb./(n0/2));
toc;

% 绘图
figure;
plot(SNRdB,ber_dB(1,:),'--r',SNRdB,ber_dB(2,:),'g--',SNRdB,ber_dB(3,:),'b--',SNRdB,ber_dB(4,:),'c--',SNRdB,ber_theory_dB,'om');
xlabel("SNR(dB)");
ylabel("BER(dB)");
